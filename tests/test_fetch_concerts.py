...
SOURCE = {
    "id": "venue",
    "name": "Venue calendar",
    "venue": {"name": "Hovet", "city": "Stockholm", "lat": 59.29, "lon": 18.08},
}
...
class CollectorTests(unittest.TestCase):
    def test_daily_collection_keeps_major_filter_cities_available(self):
        config = json.loads(
            (
                Path(__file__).resolve().parent.parent / "data" / "concert-sources.json"
            ).read_text(encoding="utf-8")
        )
        self.assertTrue(
            {"Stockholm", "Göteborg", "Malmö", "Uppsala"}.issubset(config["cities"])
        )
    ...
    def test_tickster_utc_dates_handle_swedish_midnight_and_dst(self):
        raw = {
            "id": "a",
            "name": "Gig",
            "infoUrl": "https://www.tickster.com/gig",
            "performers": ["Band"],
            "spotifyArtists": [{"name": "Band", "id": "spotify-id"}],
            "tags": ["musik", "Uppsala", "indie"],
            "state": "cancelled",
        }
        for utc, day, clock in [
            ("2026-09-08T22:30:00Z", "2026-09-09", "00:30"),
    ...
    def test_tickster_dump_filters_music_city_and_collection_products(self):
            ...
            "events": [
                {
                    ...
                    "infoUri": "https://www.tickster.com/gig",
                    "shopUri": "https://secure.tickster.com/gig",
                    "hierarchyType": "event",
                    "venueId": "venue",
                    "performers": ["Band"],
                    "tags": ["konsert", "indie"],
                ...
                },
                {
                    "id": "collection",
                    "name": "Tour",
                    "infoUri": "https://www.tickster.com/tour",
                    "hierarchyType": "collection",
                    "venueId": "venue",
                    "tags": ["konsert"],
                },
                {
                    "id": "talk",
                    "name": "Talk",
                    "infoUri": "https://www.tickster.com/talk",
                    "hierarchyType": "event",
                    "venueId": "venue",
                    "tags": ["föreläsning"],
                },
        ...
        def get(url):
            ...
            return json.dumps(
                {
                    "id": "dump-id",
                    "uri": "http://event-api-dumps.s3-eu-west-1.amazonaws.com/dump.gz?signature=temporary",
                }
            )

        def binary(url):
            self.assertTrue(
                url.startswith("https://event-api-dumps.s3-eu-west-1.amazonaws.com/")
            )
            return gzip.compress(json.dumps(dump).encode())

        result = fc.collect_tickster(
            {"cities": ["Uppsala"]}, "private-test-key", NOW, get, binary
        )
    ...
    def test_tickster_invalid_dump_is_a_failure(self):
        metadata = lambda url: json.dumps(
            {"id": "dump-id", "uri": "https://example.com/dump.gz"}
        )
        for dump in [
            {"count": 2, "events": [], "venues": []},
            {"count": 0, "events": None, "venues": []},
        ]:
            with self.assertRaises(ValueError):
                fc.collect_tickster(
                    ...
                    NOW,
                    metadata,
                    lambda url, dump=dump: json.dumps(dump).encode(),
                )

    def test_tickster_empty_dump_is_valid(self):
        rows = fc.collect_tickster(
            {"cities": ["Uppsala"]},
            "test",
            NOW,
    ...
    def katalin_fixture(self):
        source = {
            "id": "katalin",
            "indexUrl": "https://example.com/events/",
            "apiUrl": "https://example.com/wp-json/wp/v2",
            "venue": {"name": "Katalin", "city": "Uppsala", "lat": 59.86, "lon": 17.64},
            "musicGenres": ["Indie"],
        }
        ...
        rows = [
            {
                "id": 16074,
                "slug": "division-7",
                "link": "https://example.com/events/division-7/",
                "title": {"rendered": "Division 7"},
                "genre": [11],
                "acf": {
                    "date_of_event": "20260925",
                    "time": "20:00:00",
                    "ticket_status": "usual",
        ...
        def get(url):
            ...
            if "/wp-json/" not in url:
                return (
                    '<a href="?tab=2">Next</a><a href="/events/division-7/">Gig</a>'
                    if "?" not in url
                    else '<a href="/events/comedy/">Comedy</a>'
                )
            if "/genre?" in url:
                return json.dumps(
                    [{"id": 11, "name": "Indie"}, {"id": 25, "name": "Stand-up"}]
                )
    ...
    def test_katalin_follows_calendar_pages_uses_event_date_and_filters_comedy(self):
        ...
        result = events[0]
        self.assertEqual(
            (result["localDate"], result["localTime"], result["dateTime"]),
    ...
    def test_new_source_failure_keeps_ticketmaster_and_does_not_leak_credentials(self):
        ...
        with patch.dict(
            fc.os.environ,
            {"TICKETMASTER_API_KEY": "test", "TICKSTER_API_KEY": "private-test-key"},
        ):
            result = fc.refresh(
                config,
                old,
                NOW,
                ticketmaster_collector=lambda *args: [event("ticketmaster:b")],
                tickster_collector=lambda *args: (_ for _ in ()).throw(
        ...
        self.assertEqual(
            set(result["events"][0]["providers"]), {"tickster", "ticketmaster"}
        )

    def test_venue_doors_and_show_are_not_confused(self):
        raw = {
            "name": "Amon Amarth",
            "startDate": "2026-10-24T17:30:00+02:00",
            "performer": {"name": "Amon Amarth"},
            "offers": [
                {
                    "url": "https://tickets.example.com/gig",
                    "lowPrice": "395",
                    "priceCurrency": "sek",
                }
            ],
        ...
        page = fc.Page(
            "<p>Entréer öppnar <b>17:30</b></p><p>Showstart <b>18:30</b></p>"
        )
        result = fc.normalize_venue_event(
            raw, page, "https://example.com/gig", SOURCE, NOW
        )
        self.assertEqual(
            (result["localTime"], result["timeKind"], result["dateTime"]),
            ("18:30", "start", "2026-10-24T16:30:00+00:00"),
        )
    ...
    def test_ticketmaster_uses_the_cheapest_listed_price(self):
        raw = {
            ...
            "dates": {"start": {}},
            "priceRanges": [
                {"min": 595, "max": 795, "currency": "SEK"},
                {"min": 450.5, "max": 650, "currency": "SEK"},
            ],
        }
        self.assertEqual(
            fc.normalize_ticketmaster(raw, NOW)["ticketPrice"],
    ...
    def test_ticketmaster_selection_price_includes_the_displayed_service_fee(self):
        ...
        self.assertEqual(
            fc.ticketmaster_selection_price(selection),
            {"amount": 545, "currency": "SEK"},
        )
    ...
    def test_tickster_storefront_uses_the_lowest_product_price_only(self):
        ...
        self.assertEqual(
            fc.tickster_storefront_price(html), {"amount": 299.5, "currency": "SEK"}
        )

    def test_date_only_stays_without_time_or_utc_instant(self):
        result = fc.normalize_venue_event(
            {"name": "Gig", "startDate": "2026-10-24"},
            fc.Page(""),
    ...
    def test_absent_coordinates_are_not_zero(self):
        raw = {
            ...
            "name": "Gig",
            "url": "https://example.com/gig",
            "dates": {"start": {"dateTBA": True, "timeTBA": True}},
        }
    ...
    def test_cross_source_doors_and_show_records_merge_artist_and_price(self):
        ...
        ticketmaster.update(
            localTime="20:00",
            timeKind="listed",
            ticketPrice={"amount": 545, "currency": "SEK"},
        )
    ...
    def test_distinct_nights_and_products_are_preserved(self):
        ...
        self.assertEqual(
            len(fc.deduplicate([event(), event("other:b", "2026-10-25")])), 2
        )
    ...
    def test_reschedule_keeps_canonical_identity(self):
        ...
        self.assertEqual(
            (result[0]["id"], result[0]["localDate"]), ("venue:a", "2026-11-10")
        )
    ...
    def test_ticketmaster_splits_large_queries(self):
        ...
        result = fc.collect_ticketmaster(
            {"cities": ["Stockholm"], "horizonDays": 2}, "test-key", NOW, get=get
        )
    ...
    def test_calendar_schema_change_is_not_a_successful_empty_feed(self):
        source = {
            **SOURCE,
            "indexUrl": "https://example.com/gigs/",
            "eventPrefix": "https://example.com/gigs/",
        }
        with self.assertRaises(ValueError):
