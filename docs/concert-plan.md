...
- **Tickster:** authenticated access was verified on 12 September 2026. The initial v1 search/detail approach exhausted the key's hourly quota, so the adapter now uses Tickster's documented once-daily Event Dump API: one metadata request and one compressed download, followed by local city/tag filtering. It imports individual `event`/`production-child` performances tagged `musik` or `konsert`, preserving performers, tags, venues, coordinates and times while skipping collection/production containers. The first successful live collection returned 259 source records (158 Stockholm, 101 Uppsala); 257 remained in the final snapshot after horizon filtering and cross-source merging. Tags do not guarantee complete music coverage. The workflow reads and requires `TICKSTER_API_KEY` in addition to Ticketmaster's secret.
